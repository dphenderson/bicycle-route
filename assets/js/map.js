(function(window, document, undefined) {
  ...
  d3.json('/assets/json/waypoints.json', function(error, waypoints) {
    g.append('path').datum(waypoints).attr('d', path.pointRadius(2)).attr('class', 'waypoint');
  });

  d3.json('/assets/json/routes.json', function(error, routes) {
    g.append('path').datum(routes).attr('d', path).attr('class', 'route');
  });
